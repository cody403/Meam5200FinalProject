...
class IK:
    ...
    def distance_and_angle(G, H):
        ...
        relative_rotation = H[:3,:3].T @ G[:3,:3]

        # Calculate the angle from the relative rotation matrix
        internal = (np.trace(relative_rotation) - 1)/2
        angle = np.arccos(np.clip(internal, -1, 1))
    ...
    def is_valid_solution(self,q,target):
        ...
        ## STUDENT CODE STARTS HERE
        if np.any((q - IK.lower) < 0) or np.any((IK.upper - q) < 0):
            return False, "Solution not found because joint limits were exceeded"
        
        fk = FK()
        _, T_0_e = fk.forward(q)
        
        distance, angle = IK.distance_and_angle(T_0_e, target)

        if np.abs(distance) > self.linear_tol:
            return False, "Solution not found, linear toleranace exceeded"
        
        if np.abs(angle) > self.angular_tol:
            return False, "Solution not found, angular toleracne exceeded"
    ...
    def end_effector_task(q,target,method):
        ...
        if method == 'J_pseudo':
            J_used = np.linalg.pinv(J)
        elif method == 'J_trans':
            J_used = J.T
        else:
            raise ValueError("Method needs to be J_pseudo or J_trans")
        
        _, T_0_e = FK().forward(q)
        displacement, axis = IK.displacement_and_axis(target, T_0_e)
        error = np.vstack((displacement.reshape((3,1)), axis.reshape((3,1))))
        dq = J_used @ error
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            nullspace = (np.eye(7) - (J_pseudo @ J))
            dq_center = (nullspace @ dq_center)

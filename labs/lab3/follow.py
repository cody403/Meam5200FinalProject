...
class JacobianDemo():
    ...
    def ellipse(t,f=0.01,ry=.15,rz=.10):
        """
        ...
        # TODO: replace these!
        xdes = JacobianDemo.x0 + np.array([0, ry*np.cos(f*t), rz*np.sin(f*t)])
        vdes = np.array([0,-ry*f*np.sin(f*t),rz*f*np.cos(f*t)])

        angle = (pi/2)*np.sin(f*t)

        Rdes = rotvec_to_matrix(angle*np.array([1, 1, 1]))
        ang_vdes = np.array([(pi/2)*f*np.cos(f*t), 0.0, 0.0])
        ## END STUDENT CODE
    ...
    def line(t,f=0.25,L=.15):
        ...
        # TODO: replace these!
        xdes = JacobianDemo.x0 + np.array([0, 0, (L/2)*np.sin(f*t)])
        vdes = np.array([0,0,(L*f/2)*np.cos(f*t)])
